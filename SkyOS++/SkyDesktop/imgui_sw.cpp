	...
	Stats*             stats)
{
	...
	for (int i = 0; i + 3 <= pcmd.ElemCount; ) {
		...
		if (options.optimize_rectangles && i + 6 <= pcmd.ElemCount) {
			...
			ImVec2 min1, max1;
			min1.x = min3(v0.pos.x, v1.pos.x, v2.pos.x);
			min1.y = min3(v0.pos.y, v1.pos.y, v2.pos.y);
			max1.x = max3(v0.pos.x, v1.pos.x, v2.pos.x);
			max1.y = max3(v0.pos.y, v1.pos.y, v2.pos.y);
			...
			if ((v0.pos.x == min1.x || v0.pos.x == max1.x) &&
				(v0.pos.y == min1.y || v0.pos.y == max1.y) &&
				(v1.pos.x == min1.x || v1.pos.x == max1.x) &&
				(v1.pos.y == min1.y || v1.pos.y == max1.y) &&
				(v2.pos.x == min1.x || v2.pos.x == max1.x) &&
				(v2.pos.y == min1.y || v2.pos.y == max1.y) &&
				(v3.pos.x == min1.x || v3.pos.x == max1.x) &&
				(v3.pos.y == min1.y || v3.pos.y == max1.y) &&
				(v4.pos.x == min1.x || v4.pos.x == max1.x) &&
				(v4.pos.y == min1.y || v4.pos.y == max1.y) &&
				(v5.pos.x == min1.x || v5.pos.x == max1.x) &&
				(v5.pos.y == min1.y || v5.pos.y == max1.y))
			{
				const bool has_uniform_color =
					v0.col == v1.col &&
					v0.col == v2.col &&
				...
				min1.x = max(min1.x, pcmd.ClipRect.x);
				min1.y = max(min1.y, pcmd.ClipRect.y);
				max1.x = min(max1.x, pcmd.ClipRect.z - 0.5f);
				max1.y = min(max1.y, pcmd.ClipRect.w - 0.5f);

				if (max1.x < min1.x || max1.y < min1.y) { i+=6; continue; } // Completely clipped

				const auto num_pixels = (max1.x - min1.x) * (max1.y - min1.y) * target.scale.x * target.scale.y;

				if (has_uniform_color) {
					if (has_texture) {
						stats->textured_rectangle_pixels += num_pixels;
					} else {
						paint_uniform_rectangle(target, min1, max1, ColorInt(v0.col), stats);
						i += 6;
						continue;
					}
				} else {
