...
typedef void(*PSkyEmulatorMain)(SKY_FILE_Interface, SKY_ALLOC_Interface, SKY_Print_Interface, SKY_PROCESS_INTERFACE);
...
int main()
{
	...
	dllHandle = LoadLibrary("SkyOS32.dll");

	if (dllHandle == nullptr)
		return 0;

	char* pPhysicalMemory = new char[50000000];
	
	PSkyEmulatorMain kEmulationMain = (PSkyEmulatorMain)GetModuleFunction(dllHandle, "kSkyEmulatorMain");
	kEmulationMain(g_FileInterface, g_allocInterface, g_printInterface, g_processInterface);
	
	
	I_GUIEngine* pEngine = nullptr;
