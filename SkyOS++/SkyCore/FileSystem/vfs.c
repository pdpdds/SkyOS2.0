...
int vfs_mount( char * device, char * mountpoint, int fstype )
{
	...
	mount = (struct VFS_MOUNTPOINT *)kmalloc( sizeof(struct VFS_MOUNTPOINT) );
	if( mount == NULL )
		return FAIL;
	...
	mount->mountpoint = (char *)kmalloc( strlen(mountpoint)+1 );
	strcpy( mount->mountpoint, mountpoint );
	mount->device = (char *)kmalloc( strlen(device)+1 );
...
struct VFS_HANDLE * vfs_open( char * filename, int mode )
{
	...
	handle = (struct VFS_HANDLE *)kmalloc( sizeof(struct VFS_HANDLE) );
	handle->mount = mount;
	handle->mode = mode;
...
struct VFS_HANDLE * vfs_clone( struct VFS_HANDLE * handle )
{
	...
	{
		clone = (struct VFS_HANDLE *)kmalloc( sizeof(struct VFS_HANDLE) );
