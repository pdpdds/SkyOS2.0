...
int fat_file2entry( struct FAT_MOUNTPOINT * mount, char * filename, struct FAT_ENTRY * entry, struct FAT_FILE * file )
{
	...
	clusterBuffer = (BYTE *)kmalloc( mount->cluster_size );
...
int fat_updateFileEntry( struct FAT_FILE * file )
{
	...
	dir = (struct FAT_ENTRY *)kmalloc( file->mount->cluster_size );
...
{
	int root_dir_offset;
	struct FAT_MOUNTPOINT * mount = (struct FAT_MOUNTPOINT *)kmalloc( sizeof(struct FAT_MOUNTPOINT) );
	if( mount == NULL )
		return NULL;
	...
	mount->fat_data = (BYTE *)kmalloc( mount->fat_size );
	// and clear it
	memset( mount->fat_data, 0x00, mount->fat_size );
	// read in the FAT
	vfs_read( mount->device, (void *)mount->fat_data, mount->fat_size );
	// read in root directory
	mount->rootdir = (struct FAT_ENTRY *)kmalloc( mount->bootsector.num_root_dir_ents * sizeof( struct FAT_ENTRY ) );
...
struct VFS_HANDLE * fat_open( struct VFS_HANDLE * handle, char * filename )
{
	...
	if( (fat_mount = (struct FAT_MOUNTPOINT *)handle->mount->data_ptr) == NULL )
		return NULL;
	file = (struct FAT_FILE *)kmalloc( sizeof(struct FAT_FILE) );
...
int fat_rw( struct VFS_HANDLE * handle, BYTE * buffer, DWORD size, int mode )
{
	...
	clusterBuffer = (BYTE *)kmalloc( file->mount->cluster_size );
	// read/write the data
	while( TRUE )
	{
...
int fat_create( struct VFS_MOUNTPOINT * mount, char * filename )
{
	...
	dirname = (char *)kmalloc( strlen(filename)+1 );
		...
		if( fat_file2entry( fat_mount, dirname, NULL, file ) == SUCCESS )
		{
			...
			if( fat_setFileName( file, filename ) == SUCCESS )
			{
				...
				if( create_dir )
				{
					...
					{
						struct FAT_ENTRY * entry = (struct FAT_ENTRY *)kmalloc( file->mount->cluster_size );
						...
						printf("[create] start_cluster fail\n");
						kfree( dirname );
						kfree( file );
						return FAIL;
					}
...
struct VFS_DIRLIST_ENTRY * fat_list( struct VFS_MOUNTPOINT * mount, char * dirname )
{
	...
	{
		dir = (struct FAT_ENTRY *)kmalloc( fat_mount->cluster_size );
	...
	entry = (struct VFS_DIRLIST_ENTRY *)kmalloc( sizeof(struct VFS_DIRLIST_ENTRY)*32 );
	// clear it
...
int fat_init( void )
{
	...
	fs = (struct VFS_FILESYSTEM *)kmalloc( sizeof(struct VFS_FILESYSTEM) );
