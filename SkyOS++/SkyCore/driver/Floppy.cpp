...
int Floppy::ReadSector(int lba, unsigned physaddr)
{
	...
	// controller.
	OutPortByte(kDmaCommand, 0x14);	// Disable DMA
	OutPortByte(kDmaMode, 0x56);
	OutPortByte(kDmaFlipFlop, 0);
	OutPortByte(kDmaAddress, physaddr & 0xff);			// LSB
	OutPortByte(kDmaAddress, (physaddr >> 8) & 0xff);	// MSB
	OutPortByte(kDmaPage, (physaddr >> 16) & 0xff);		// Page
	OutPortByte(kDmaFlipFlop, 0);
	OutPortByte(kDmaCount, 0xff);	// Count LSB (512 bytes)
	OutPortByte(kDmaCount, 1);		// Count MSB
	OutPortByte(kDmaChannelMask, 2);	// Release channel 2
	OutPortByte(kDmaCommand, 0x10);	// Reenable DMA
...
{
	OutPortByte(fBaseAddress + offset, value);
}

int Floppy::ReadRegister(RegisterOffset offset)
{
	return InPortByte(fBaseAddress + offset);
}
