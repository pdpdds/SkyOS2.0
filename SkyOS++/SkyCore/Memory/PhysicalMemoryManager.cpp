...
{	
	...
	void SetMemoryAvailable(UINT32 base, size_t size, bool unset = true);

	void SetBit(int bit);
	void UnsetBit(int bit);
	bool TestMemoryMap(int bit);

	void BuildKernelMemoryLayout()
	{
	...
	void Initialize()
	{
		printf("Physical Memory Manager Init..\n");
...
#else			
		m_pMemoryMap = (UINT32*)(bootParams._memoryLayout._kernelBase + bootParams._kernelSize);
#endif 	

		//메모리맵 크기
		m_memoryMapSize = m_maxBlocks / PMM_BITS_PER_INDEX;

		BuildKernelMemoryLayout();		
		...
		/*for (int i = 0; i < bootParams._rangeCount; i++)
		{
			...
			if(bootParams.allocatedRange[i].type == MEMORY_REGION_AVAILABLE && i != 0)
				SetMemoryAvailable(begin, end - begin);
		}*/

		//사용할 수 있는 공간을 UnSet한다.
		UINT32 availablePoint = (UINT32)m_pMemoryMap + m_memoryMapSize * sizeof(UINT32);
		SetMemoryAvailable(availablePoint, bootParams._memorySize - availablePoint);
...
#ifdef SKY_EMULATOR
		UINT32 baseIOMemory = (UINT32)m_pMemoryMap + m_memoryMapSize * sizeof(UINT32);
		UINT32 addr = baseIOMemory + frame * PMM_BLOCK_SIZE;
#else
		UINT32 baseIOMemory = (UINT32)m_pMemoryMap + m_memoryMapSize * sizeof(UINT32);
	...
	void SetMemoryAvailable(UINT32 base, size_t size, bool unset)
	{
