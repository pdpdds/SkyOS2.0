...
{
	B_SET_CHANNEL(data->enable_bits, 0, 1);
	B_SET_CHANNEL(data->enable_bits, 1, 1);
	B_SET_CHANNEL(data->enable_bits, 2, 1);
	B_SET_CHANNEL(data->enable_bits, 3, 1);
...
//		dprintf("%s: Error setting up recording buffers (%s)\n", __func__, strerror(rc));
	...
	for (bidx=0; bidx < data->return_playback_buffers; bidx++) {
		for (cidx=0; cidx < data->return_playback_channels; cidx++) {
			data->playback_buffers[bidx][cidx].base = (int *)dev->playback_stream.buffers[bidx] + (playback_sample_size * cidx);
	...
	for (bidx=0; bidx < data->return_record_buffers; bidx++) {
		for (cidx=0; cidx < data->return_record_channels; cidx++) {
			data->record_buffers[bidx][cidx].base = (int *)dev->record_stream.buffers[bidx] + (record_sample_size * cidx);
