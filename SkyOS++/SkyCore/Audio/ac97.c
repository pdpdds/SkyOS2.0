	...
	ushort subvendor_id, ushort subsystem_id)
{
	...
	*_dev = dev = (ac97_dev *)kmalloc(sizeof(ac97_dev));
...
ac97_set_rate(ac97_dev *dev, uint8 reg, uint32 rate)
{
	...
	uint32 temp = 48000ULL / dev->clock;
	value = (uint32)(rate * temp); /* need 64 bit calculation for rates 96000 or higher */
...
ac97_get_rate(ac97_dev *dev, uint8 reg, uint32 *rate)
{
	...
	uint32 temp = (uint64)dev->clock / 48000;
	*rate = (uint32)(value * temp); /* need 64 bit calculation to avoid overflow*/
	return true;
}
