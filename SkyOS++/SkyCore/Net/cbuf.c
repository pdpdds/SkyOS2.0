...
cbuf *cbuf_duplicate_chain(cbuf *chain, size_t offset, size_t len, size_t leading_space)
{
	...
	if(leading_space > 0) {
		cbuf_truncate_head(newbuf, leading_space, 0);
	}
...
cbuf *cbuf_truncate_head(cbuf *buf, size_t trunc_bytes, char free_unused)
{
	cbuf *head = buf;
	cbuf *tail = NULL;
...
int cbuf_truncate_tail(cbuf *buf, size_t trunc_bytes, char free_unused)
{
	size_t offset;
	cbuf *head = buf;
