...
static int ipv4_route_add_etc(ipv4_addr network_addr, ipv4_addr netmask, ipv4_addr if_addr, if_id interface_num, int flags, ipv4_addr gw_addr)
{
	...
	e = (ipv4_routing_entry *)kmalloc(sizeof(ipv4_routing_entry));
	if(!e)
...
static ipv4_fragment *ipv4_create_frag_struct(ipv4_fragment_key *key, cbuf *buf, uint16 offset, uint16 len, char last_frag)
{
	ipv4_fragment *frag;

	// create a new frag
	frag = (ipv4_fragment *)kmalloc(sizeof(ipv4_fragment));
	if(!frag)
		return NULL;
...
static int ipv4_process_frag(cbuf *inbuf, ifnet *i, cbuf **outbuf)
{
	...
	*outbuf = NULL;
	...
	last_frag = (ntohs(header->flags_frag_offset) & IPV4_FLAG_MORE_FRAGS) ? 0 : 1;
...
//			dprintf("ipv4_process_frag: received fragment is bad\n");
		...
		if(last && temp) {
			if(last->offset + len == temp->offset) {
				// merge them
				ipv4_header *next_header = cbuf_get_ptr(temp->buf, 0);
				temp->buf = cbuf_truncate_head(temp->buf, ((next_header->version_length & 0xf) * 4), 1);
...
//			dprintf("ipv4 packet too short (buf_len %ld, packet len %d)\n", buf_len, packet_len);
		...
		if(buf_len > packet_len) {
			cbuf_truncate_tail(buf, buf_len - packet_len, 1);
		}
	}
	...
	buf = cbuf_truncate_head(buf, (header->version_length & 0xf) * 4, 1);
