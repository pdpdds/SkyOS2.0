...
int MapFileReader::readFile(char	*fileName)
{
	...
	// open the map file
	fp = g_FileInterface.sky_fopen(fileName, "r");
	if (fp != NULL)
	{
...
*/
		...
		{
			if (r)
				g_printInterface.sky_printf("Parsing C and C++ decorated names complete\n");
			else
				g_printInterface.sky_printf("Parsing C and C++ decorated names failed\n");
		}
		g_FileInterface.sky_fclose(fp);
	}
	else
	{
		if (verbose)
			g_printInterface.sky_printf("Failed to open map file %s\n", fileName);

		r = FALSE;
	}

	g_printInterface.sky_printf("success to open map file %s\n", fileName);
								      ...
								      int	len)
{
	...
	do 
	{
		p = g_FileInterface.sky_fgets(line, (len - 1), fp);

		if (p == NULL)
		{
			// check if at end of file, if not, then we must be at a \n character

			if (g_FileInterface.sky_feof(fp) != 0)
				break;				// end of file

			// move past the \n character

			g_FileInterface.sky_fseek(fp, 1, SEEK_CUR);
		}
...
int MapFileReader::readSectionInformation(FILE	*fp)
{
	...
	while(TRUE)
	{
		p = g_FileInterface.sky_fgets(line, (LINE_LENGTH - 1), fp);
		if (p == NULL)
			return FALSE;
...
int MapFileReader::readPublicSymbols(FILE	*fp)
{
	...
	if (verbose)
		g_printInterface.sky_printf("Searching for start of C and C++ exported names\n");
							   ...
							   int	addToPublicSymbols)
{
	...
	if (verbose)
		g_printInterface.sky_printf("Parsing C and C++ export names\n");
