...
typedef status_t (*device_open_hook)(const char *name, uint32_t flags, void **cookie);
...
//typedef status_t (*device_select_hook) (void *cookie, uint8_t event, uint32 ref,
  //                                      selectsync *sync);
//typedef status_t (*device_deselect_hook) (void *cookie, uint8_t event,
  //                                        selectsync *sync);
...
typedef struct {
	...
	char	removable;				/* non-zero if removable */
	char	read_only;				/* non-zero if read only */
	char	write_once;				/* non-zero if write-once */
} device_geometry;
