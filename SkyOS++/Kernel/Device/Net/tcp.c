...
static void handle_ack(tcp_socket *s, uint32 sequence, uint32 window_size, char with_data);
...
static tcp_socket *create_tcp_socket(void)
{
	...
	s = (tcp_socket*)kmalloc(sizeof(tcp_socket));
	if(!s)
		return NULL;
...
ssize_t tcp_recvfrom(void *prot_data, void *buf, ssize_t len, sockaddr *saddr, int flags, bigtime_t timeout)
{
	...
	s->read_buffer = cbuf_truncate_head(s->read_buffer, to_copy, 1);
...
//			mutex_lock(&s->lock);
		...
		if(s->write_buffer != NULL) {
			...
			if(err < 0) {
				cbuf_truncate_tail(s->write_buffer, chunk_size, 1);
				sent = err;
				goto out;
			}
		} else {
...
//				dprintf("tcp: data was acked that we didn't send\n");
			...
			s->write_buffer = cbuf_truncate_head(s->write_buffer, ack_len, 1);
...
//	ASSERT_LOCKED_MUTEX(&s->lock);
	...
	if(SEQUENCE_LTE(seq_low, s->rx_win_low) && SEQUENCE_GTE(seq_high, s->rx_win_low)) {
		...
		buf = cbuf_truncate_head(buf, header_length + (s->rx_win_low - seq_low), 1);
...
//		sem_release(s->read_sem, 1);
		...
		while(s->reassembly_q) {
			...
			if(SEQUENCE_LT(packet_high, s->rx_win_low)) {
				...
				tmp = cbuf_truncate_head(tmp, packet_header_len + (s->rx_win_low - packet_low), 1);
