	...
	uint8 function, uint16 deviceId)
{
	...
	TRACE("jmicron_fixup_ahci: domain %u, bus %u, device %u, function %u, "
	...
	TRACE("jmicron_fixup_ahci: Register 0x40 : 0x%08" B_PRIx32 "\n", val);
	...
	TRACE("jmicron_fixup_ahci: Assigning IRQ %d at device "
		"function 1.\n", irq);
	...
	uint8 function, uint16 deviceId)
{
	...
	switch (deviceId) {
		case 0x2825: // ICH8 Desktop when in IDE emulation mode
			TRACE("intel_fixup_ahci: WARNING found ICH8 device id 0x2825\n");
			return;
		case 0x2926: // ICH9 Desktop when in IDE emulation mode
			TRACE("intel_fixup_ahci: WARNING found ICH9 device id 0x2926\n");
			return;
	...
	TRACE("intel_fixup_ahci: domain %u, bus %u, device %u, function %u, "
		"deviceId 0x%04x\n", domain, bus, device, function, deviceId);

	TRACE("intel_fixup_ahci: 0x24: 0x%08" B_PRIx32 "\n",
		pci->ReadConfig(domain, bus, device, function, 0x24, 4));
	TRACE("intel_fixup_ahci: 0x90: 0x%02" B_PRIx32 "\n",
	...
	if ((map >> 6) == 0) {
		...
		TRACE("intel_fixup_ahci: switching from IDE to AHCI mode\n");
		...
		TRACE("intel_fixup_ahci: ide-bar5 bits-1: 0x%08" B_PRIx32 "\n",
			pci->ReadConfig(domain, bus, device, function, 0x24, 4));
		pci->WriteConfig(domain, bus, device, function, 0x24, 4, 0);
		TRACE("intel_fixup_ahci: ide-bar5 bits-0: 0x%08" B_PRIx32 "\n",
			pci->ReadConfig(domain, bus, device, function, 0x24, 4));
		...
		pci->WriteConfig(domain, bus, device, function, 0x24, 4, 0xffffffff);
		TRACE("intel_fixup_ahci: ahci-bar5 bits-1: 0x%08" B_PRIx32 "\n",
			pci->ReadConfig(domain, bus, device, function, 0x24, 4));
		pci->WriteConfig(domain, bus, device, function, 0x24, 4, 0);
		TRACE("intel_fixup_ahci: ahci-bar5 bits-0: 0x%08" B_PRIx32 "\n",
			pci->ReadConfig(domain, bus, device, function, 0x24, 4));
	...
	uint16 deviceId)
{
	...
	TRACE("ati_fixup_ixp: domain %u, bus %u, device %u, function %u, deviceId 0x%04x\n",
		domain, bus, device, function, deviceId);

	TRACE("ati_fixup_ixp: found IXP chipset, forcing IRQ 0 as level triggered.\n");
	// XXX: maybe use pic_*() ?
	...
	uint8 function)
{
	...
	TRACE("pci_fixup_device: domain %u, bus %u, device %u, function %u\n",
